...
class Fcoin():
    ...
    def auth(self, key, secret):
        self.key = bytes("xxxx",'utf-8')
        self.secret = bytes("xxxx", 'utf-8')
    ...
    def signed_request(self, method, api_url, **payload):
        ...
        try:
            ...
            print(err)
            if r.text.find('account balance insufficient')>-1:
                print(r.text+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+'---->Insufficient balance')
        if r.status_code == 200:
            return r.json()
